...
def señal_senoidal(freq=440.0, fs=8000, duracion=0.5, amplitud=1.0):
    n = np.arange(int(fs * duracion))
    return amplitud * np.sin(2 * np.pi * freq * n / fs)

def señal_suma_senoidales(freqs, amplitudes, fs=8000, duracion=0.5):
    n = np.arange(int(fs * duracion))
    return sum(A * np.sin(2 * np.pi * f * n / fs) for f, A in zip(freqs, amplitudes))

def señal_cuadrada(freq=440.0, fs=8000, duracion=0.5, amplitud=1.0):
...
def main():
    ...
    if op == '1':
        ...
        rs.append(comprimir_señal(señal_senoidal(f), 8000, fra, f"senoidal_{int(f)}Hz"))

    elif op == '2':
        rs.append(comprimir_señal(
            señal_suma_senoidales([261, 329, 392, 523, 659], [1.0, 0.8, 0.9, 0.4, 0.3]),
            8000, fra, "acorde_doMiSol"))

    elif op == '3':
        f = float(input("  Frecuencia fundamental (Hz) [220]: ").strip() or "220")
        rs.append(comprimir_señal(señal_cuadrada(f), 8000, fra, f"cuadrada_{int(f)}Hz"))

    elif op == '4':
        rs.append(comprimir_señal(señal_ruido_blanco(4096), 8000, fra, "ruido_blanco"))
        ...
        for nombre, señal, fs in [
            ("senoidal_440Hz",   señal_senoidal(440),                                   8000),
            ("acorde_doMiSol",   señal_suma_senoidales([261, 329, 392, 523, 659],
                                                        [1.0, 0.8, 0.9, 0.4, 0.3]),    8000),
            ("cuadrada_220Hz",   señal_cuadrada(220),                                   8000),
            ("chirp_100_2000Hz", señal_chirp(),                                         8000),
            ("ruido_blanco",     señal_ruido_blanco(4096),                              8000),
        ]:
            rs.append(comprimir_señal(señal, fs, fra, nombre))
    else:
